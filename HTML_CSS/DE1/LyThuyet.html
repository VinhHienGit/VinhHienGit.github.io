<!DOCTYPE html>
<html>
  <head>
    <title>Page Title</title>
    <link rel="stylesheet" href="styles.css">
    <style type="text/css">
    </style>
  </head>
  <body>
  	<!-- -->
    <div id="container">
    	<!--Player profile -->
      <div class="profile">
      	<h3 class="title">Player Profile</h3>
      	<div class="player-profile">
      	  <img src="img/avatar.gif" alt="avartar player">
      	  <ul class="player-profile-info">
      	  	<li>Ten: Nguyen Xuan Nam</li>
      	  	<li>Noi sinh: Ha Noi</li>
      	  	<li>Ngay sinh: 28/11/1990</li>
      	  	<li>Vi tri: Teamlear</li>
      	  	<li>Cao: 170cm</li>
      	  	<li>Nang: 65kg</li>
      	  </ul>
      	</div>
      </div>
      <!--Facts -->
      <div class="facts">
      	<h3 class="title">Facts</h3>
      	<ul class="facts-contain">
      		<li>Nam gia nhap The Cong nam 1999</li>
      		<li>Trong suot thoi trai tre gia nhap The cong Nam da ghi duoc 160 ban thang trong vong 7 nam</li>
      		<li>Nam choi tran dau tien cho U21 Viet Nam vao do tuoi 16</li>
      		<li>Nam da doat cup U21 cho Viet Nam vao nam 2007</li>
      		<li>Nam da dat cup cau thu xuat sac nhat trong nam</li>
      	</ul>
      </div>
      <!--Post -->
      <div class="post">
      	<div class="post-contain">
      		Nam choi cho cau lac bo the cong.<br>
      		The cong dat cup vo dich"
      	</div>
      	<p class="post-info">
      		Posted by
      		<a href="#">The cong's Fan</a>
      		 20/12/2007.
      	</p>
      	<div class="post-contain">
      		Nam la cau thu choi hay nhat The cong.<br>
      		Va anh ta da chuyen toi Hoa phat
      	</div>
      	<p class="post-info">
      		Posted by
      		<a href="#">The cong's Fan</a>
      		 20/12/2007.
      	</p>
      </div>
      <!--Comment -->
      <dir class="comment">
      	<h4>Your comment</h4>
      	<form>
      		<table>
      			<tbody>
      				<tr>
      					<td>Your name: </td>
      					<td>
      						<input type="text" name="name">
      					</td>
      				</tr>
      				<tr>
      					<td>Your email(will not public)</td>
      					<td>
      						<input type="text" name="email">
      					</td>
      				</tr>
      				<tr>
      					<td>Comment: </td>
      					<td>
      						<textarea name="comment" cols="50" rows="5">Your comment here &#60/textarea>
      						</textarea>
      					</td>
      				</tr>
      			</tbody>
      		</table>
      	</form>
      </dir>
    </div>
  </body>
</html>